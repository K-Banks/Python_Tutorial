...
def get_metrics(linear_classifier):
    evaluation_metrics = linear_classifier.predict(input_fn = predict_validation_input_fn)
    print("AUC on validation set is %0.2f" % evaluation_metrics["auc"])
    print("Accuracy on validation set is %0.2f" % evalution_metrics["accuracy"])
    validation_probabilities = linear_classifier.predict(input_fn=predict_validation_input_fn)
    ...
    plot.plot([0,1], [0,1], label="random classifier")
    plt.legend(loc=2)
